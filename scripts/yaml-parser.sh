...
_yaml_deep_merge() {
    ...
    base_keys=$(_yaml_get_top_keys "$base" "$depth")
    overlay_keys=$(_yaml_get_top_keys "$overlay" "$depth")
    all_keys=$(echo -e "$base_keys\n$overlay_keys" | sort -u)
...
"
            ...
            local base_has_nested
            local overlay_has_nested
            base_has_nested=$(_yaml_has_nested_content "$base_section")
            overlay_has_nested=$(_yaml_has_nested_content "$overlay_section")
            
            if [[ "$base_has_nested" == "true" && "$overlay_has_nested" == "true" ]]; then
                local base_is_list
                local overlay_is_list
                base_is_list=$(_yaml_is_list_section "$base_section")
                overlay_is_list=$(_yaml_is_list_section "$overlay_section")
...
"
                    ...
                    local base_nested
                    local overlay_nested
                    local merged_nested
                    base_nested=$(_yaml_get_nested_content "$base_section")
                    overlay_nested=$(_yaml_get_nested_content "$overlay_section")
                    merged_nested=$(_yaml_deep_merge "$base_nested" "$overlay_nested" $((depth + 1)))
                    local indent
