...
setup_docker_repo() {
    ...
    if gpg --no-default-keyring --keyring "$tmp_keyring" --list-keys --with-colons 2>/dev/null | grep -q "$DOCKER_GPG_FINGERPRINT"; then
        ...
        gpg --no-default-keyring --keyring "$tmp_keyring" --export --output "$DOCKER_KEYRING" 2>/dev/null
    ...
    echo "deb [arch=$arch signed-by=$DOCKER_KEYRING] https://download.docker.com/linux/${platform} $codename stable" | \
